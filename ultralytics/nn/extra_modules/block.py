...
class ODConv2d(nn.Module):
    ...
    def _forward_impl_common(self, x):
        ...
        # 核心修改：如果是 kernel_num=1，直接去掉维度，不要用 sum
        if self.kernel_num == 1:
            aggregate_weight = aggregate_weight.squeeze(1)  # 去掉 kernel_num 维度
        else:
            aggregate_weight = torch.sum(aggregate_weight, dim=1)

        # 核心修改：明确去掉 batch 维度 (因为 batch 总是 1)，让权重变成纯粹的 4D 张量
        # 这样 F.conv2d 接收到的 weight 就是 [16, 32, 3, 3]，不再有那个多余的 [1, ...]
        aggregate_weight = aggregate_weight.squeeze(0)

        # 4. 再次强制 reshape，确保万无一失
        aggregate_weight = aggregate_weight.view(
            self.out_planes, self.in_planes // self.groups, self.kernel_size, self.kernel_size)

        # 5. 卷积运算 (这里的 groups 必须是静态的)
    ...
    def _forward_impl_pw1x(self, x):
        ...
        # 这里也要确保权重是 4D 的
        weight = self.weight.view(self.out_planes, self.in_planes // self.groups, 1, 1)

        output = F.conv2d(x, weight=weight, bias=None, stride=self.stride,
                          padding=self.padding, dilation=self.dilation, groups=self.groups)
        output = output * filter_attention
        return output
